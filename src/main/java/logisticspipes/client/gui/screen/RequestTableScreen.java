...
public class RequestTableScreen extends LogisticsBaseGuiScreen<RequestTableMenu>
    ...
    public void init() {
        ...
        addRenderableWidget(
            hideWhileSmall.addChain(wire(new SmallGuiButton(2, right - 90, topPos + 5, 10, 10, "<"), 2))); // Prev page
        addRenderableWidget(
            hideWhileSmall.addChain(wire(new SmallGuiButton(10, right - 148, bottom - 15, 26, 10, "---"), 10))); // -64
        addRenderableWidget(
            hideWhileSmall.addChain(wire(new SmallGuiButton(4, right - 148, bottom - 26, 15, 10, "--"), 4))); // -10
        addRenderableWidget(
            hideWhileSmall.addChain(wire(new SmallGuiButton(5, right - 132, bottom - 26, 10, 10, "-"), 5))); // -1
        addRenderableWidget(
            hideWhileSmall.addChain(wire(new SmallGuiButton(6, right - 86, bottom - 26, 10, 10, "+"), 6))); // +1
        addRenderableWidget(
            hideWhileSmall.addChain(wire(new SmallGuiButton(7, right - 74, bottom - 26, 15, 10, "++"), 7))); // +10
        addRenderableWidget(
            hideWhileSmall.addChain(wire(new SmallGuiButton(11, right - 86, bottom - 15, 26, 10, "+++"), 11))); // +64
        popupCheck = new GuiCheckBox(8, leftPos + 209, bottom - 60, 14, 14,
            LPConfigs.COMMON.DISPLAY_POPUP.getAsBoolean());
        ...
        addRenderableWidget(
            moveWhileSmall.addChain(wire(new SmallGuiButton(15, leftPos + 108, topPos + 53, 15, 10, "++"), 15))); // +10
        addRenderableWidget(
            moveWhileSmall.addChain(wire(new SmallGuiButton(16, leftPos + 96, topPos + 64, 26, 10, "+++"), 16))); // +64

        addRenderableWidget(
            moveWhileSmall.addChain(wire(new SmallGuiButton(30, leftPos + 96 + 2, topPos + 18, 10, 10, "X"), 30))); // x
        addRenderableWidget(moveWhileSmall.addChain(
        ...
        (cycleButtons[0] = addRenderableWidget(
            wire(new SmallGuiButton(21, leftPos + 124, topPos + 30, 15, 10, "⏶"), 21))).visible = false;
        (cycleButtons[1] = addRenderableWidget(
            wire(new SmallGuiButton(22, leftPos + 124, topPos + 42, 15, 10, "⏷"), 22))).visible = false;
        ...
        if (itemDisplay == null) {
            itemDisplay = new ItemDisplay(this, font, this, this, leftPos + 205, topPos + 18, 200, panelHeight - 100,
                right - 104, bottom - 24, 36, new int[] { 1, 10, 64, 64 }, true);
        }
    ...
    public void extractGuiBackground(GuiGraphicsExtractor guiGraphics, int i, int j, float f) {
        ...
        LPGuiGraphics.drawGuiBackGround(guiGraphics, leftPos, topPos, right - (showRequest ? 0 : 105), bottom, 0.0f,
            true);
        ...
        for (int a = 0; a < 10; a++) {
            guiGraphics.fill(leftPos + 97 - a, topPos + 40 - a, leftPos + 98 - a, topPos + 41 + a,
                Color.getValue(Color.DARKER_GREY));
        }
        for (int a = 0; a < 15; a++) {
            guiGraphics.fill(leftPos + 164 + a, topPos + 51 + a, leftPos + 166 + a, topPos + 53 + a,
                Color.getValue(Color.DARKER_GREY));
            guiGraphics.fill(leftPos + 164 + a, topPos + 65 - a, leftPos + 166 + a, topPos + 67 - a,
                Color.getValue(Color.DARKER_GREY));
        }
            ...
            if (!handledExtension.get(entry.getKey())) {
                ...
                extensionControllerLeft.addExtension(new GuiExtension() {
                    ...
                    public void renderForeground(GuiGraphicsExtractor guiGraphics, int left, int top) {
                        ...
                        // Draw number
                        guiGraphics.text(minecraft.font, s, left + 22 - minecraft.font.width(s), top + 14, 0xFFFFFFFF,
                            true);
                        if (isFullyExtended()) {
                            ...
                            for (IOrderInfoProvider order : list) {
                                ...
                                s = TextUtil.getThreeDigitFormattedNumber(stack.getCount(), false);
                                // Draw number
                                guiGraphics.text(minecraft.font, s, x + 17 - minecraft.font.width(s), y + 9, 0xFFFFFFFF,
                                    true);
                            ...
                            ordererPosition.keySet().stream()
                                .filter(key -> xPos >= key.getValue1() && xPos < key.getValue1() + 18 && yPos >= key
                                    ...
                                    guiGraphics.setComponentTooltipForNextFrame(font,
                                        list.stream().map(Component::literal).collect(Collectors.toList()), xPos, yPos);
                                });
                        } else if (entry.getValue() != null && entry.getValue().getValue1() != null
                            && entry.getValue().getValue1().getDisplayItem() != null) {
                            guiGraphics.setTooltipForNextFrame(font,
                                entry.getValue().getValue1().getDisplayItem().makeNormalStack(), xPos, yPos);
                        }
                    }
                });
            }
    ...
    public void extractLabels(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
        ...
        macroButton.active =
            !table.diskInv.getItem(0).isEmpty() && table.diskInv.getItem(0).getItem().equals(LPItems.DISK.get());
    ...
    public void handleRequestAnswer(Collection<IResource> items, boolean error, ISubGuiController control,
        Player player) {
    ...
    public void handleSimulateAnswer(Collection<IResource> used, Collection<IResource> missing,
        ISubGuiController control, Player player) {
        while (control.hasSubGui()) {
            control = control.getSubGui();
        }
