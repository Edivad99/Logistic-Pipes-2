...
public class SecurityStationScreen extends LogisticsBaseGuiScreen<SecurityStationMenu> implements PlayerListReceiver {
    ...
    public SecurityStationScreen(SecurityStationMenu menu, Inventory inventory, Component title) {
    ...
    public void init() {
        ...
        btnMinusMinus.setPressListener(b -> ClientPacketDistributor.sendToServer(
            new SecurityCardActionMessage(tile.getBlockPos(), CardAction.CLEAR)));
        addRenderableWidget(btnMinusMinus);
        btnMinusMinus.visible = false;
        btnMinus = new SmallGuiButton(1, leftPos + 10, topPos + 139, 30, 20, "-");
        btnMinus.setPressListener(b -> ClientPacketDistributor.sendToServer(
            new SecurityCardActionMessage(tile.getBlockPos(), CardAction.TAKE_ONE)));
        addRenderableWidget(btnMinus);
        btnPlus = new SmallGuiButton(2, leftPos + 45, topPos + 139, 30, 20, "+");
        btnPlus.setPressListener(b -> ClientPacketDistributor.sendToServer(
            new SecurityCardActionMessage(tile.getBlockPos(), CardAction.GIVE_ONE)));
        addRenderableWidget(btnPlus);
        btnPlusPlus = new SmallGuiButton(3, leftPos + 140, topPos + 179, 30, 20, "++");
        btnPlusPlus.setPressListener(b -> ClientPacketDistributor.sendToServer(
            new SecurityCardActionMessage(tile.getBlockPos(), CardAction.GIVE_STACK)));
        ...
        if (searchBar == null) {
            searchBar = new InputBar(this.font, this, leftPos + 180, bottom - 120, right - 8 + addition - leftPos - 180,
                17);
    ...
    protected void extractGuiBackground(GuiGraphicsExtractor guiGraphics, int var2, int var3, float var1) {
        ...
        for (String player : players) {
            ...
            if (leftPos + 180 < lastClickedX && lastClickedX < leftPos + 280 && pos - 11 < lastClickedY
                && lastClickedY < pos) {
                lastClickedX = -10000000;
                lastClickedY = -10000000;
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        double j = event.y();
        int k = event.button();
        if ((i >= leftPos + 5 && i < right - 5 + addition && j >= topPos + 5 && j < bottom - 5)
            && !searchBar.isFocused()) {
            lastClickedX = (int) i;
            lastClickedY = (int) j;
            lastClickedK = k;
