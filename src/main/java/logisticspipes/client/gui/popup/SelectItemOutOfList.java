...
public class SelectItemOutOfList extends SubGuiScreen implements IItemSearch {
    ...
    public void init() {
        ...
        if (search == null) {
            search = new InputBar(font, this.getBaseScreen(), guiLeft + 7, bottom - 23, right - guiLeft - 64, 15,
                false);
        }
        ...
        if (itemDisplay == null) {
            itemDisplay = new ItemDisplay(this, font, this.getBaseScreen(), null, guiLeft + 10, guiTop + 18, xSize - 20,
                ySize - 48, 0, 0, 0, new int[] { 1, 10, 64, 64 }, true);
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        if (itemDisplay.handleClick((int) mouseX, (int) mouseY, button)) {
            return true;
        }
        if (search.handleClick((int) mouseX, (int) mouseY, button)) {
            return true;
        }
