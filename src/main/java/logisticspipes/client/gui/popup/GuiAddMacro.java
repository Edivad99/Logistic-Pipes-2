...
public class GuiAddMacro extends SubGuiScreen implements IItemSearch {
    ...
    private void loadMacroItems() {
        ...
        for (int i = 0; i < inventar.size(); i++) {
            ItemIdentifierStack stack = ItemIdentifierStack.loadFromNBT(inventar.getCompoundOrEmpty(i),
    ...
    protected void extractLabels(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
        ...
        for (ItemIdentifierStack itemIdStack : diskProvider.getItemDisplay().allItems) {
            ...
            if (!super.hasSubGui()) {
                if (mouseX >= x && mouseX < x + panelXSize && mouseY >= y && mouseY < y + panelYSize) {
                    ...
                    guiGraphics.fill(x - 2, y, x + panelXSize - 4, y + panelYSize - 4,
                ...
                if (mousePosX != 0 && mousePosY != 0) {
                    if ((mousePosX >= x && mousePosX < x + panelXSize && mousePosY >= y && mousePosY < y + panelYSize)
                        || (mouseX >= x && mouseX < x + panelXSize && mouseY >= y && mouseY < y + panelYSize && (
        ...
        ItemStackRenderer.renderItemIdentifierStackListIntoGui(guiGraphics, diskProvider.getItemDisplay().allItems,
            this, pageAll, guiLeft + 9, guiTop + 17, 9, 45, panelXSize, panelYSize, 100.0F, DisplayAmount.NEVER);
        ...
        ItemStackRenderer.renderItemIdentifierStackListIntoGui(guiGraphics, macroItems, this, pageMacro, guiLeft + 10,
            guiTop + 150, 9, 9, panelXSize, panelYSize, 100.0F, DisplayAmount.ALWAYS);
    }
    ...
    protected void extractGuiBackground(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
        ...
        guiGraphics.text(minecraft.font, "Add Macro", guiLeft + minecraft.font.width("Add Macro") / 2, guiTop + 6,
            0xFF404040, false);
        ...
        guiGraphics.text(minecraft.font, pageString1, right - 47 - minecraft.font.width(pageString1) / 2, guiTop + 6,
            0xFF404040, false);

        guiGraphics.text(minecraft.font, "Macro Items", guiLeft + minecraft.font.width("Add Macro") / 2, guiTop + 136,
            0xFF404040, false);
        ...
        guiGraphics.text(minecraft.font, pageString2, right - 47 - minecraft.font.width(pageString2) / 2, guiTop + 136,
            0xFF404040, false);
