...
public class RequestMonitorPopup extends SubGuiScreen {
    ...
    @Override
    protected void extractLabels(GuiGraphicsExtractor guiGraphics, int par1, int par2) {
    ...
    private void saveTreeToImage() {
        ...
        if (minecraft.player != null) {
            minecraft.player.sendSystemMessage(
                Component.literal("Tree view export has not been ported to this Minecraft version"));
        }
        ...
        // //GuiGraphicsExtractor oldStored = getGuiGraphics();
        //         GuiGraphicsExtractor gg = new GuiGraphicsExtractor(minecraft, minecraft.renderBuffers().bufferSource());
            ...
            while (true) {
                ...
                if (!candidate.exists()) {
                    ...
                    Minecraft.getInstance().player.sendSystemMessage(
                        Component.literal("Saved tree view as " + candidate.getName()));
                    return;
                }
                ++i;
            }
    ...
    private void drawMap(GuiGraphicsExtractor guiGraphics, int par1, int par2) {
        ...
        LinkedLogisticsOrderList list = table.watchedRequests.get(orderId).getValue2();
        if (!list.isEmpty()) {
            SimpleGraphics.drawVerticalLine(guiGraphics, innerLeftSide - mapX + 110, innerTopSide - mapY - 197,
                innerTopSide - mapY - 180, Color.GREEN, zoom.line);
        }
        ...
        String s = Integer.toString(orderId);
        if (!list.isEmpty()) {
            guiGraphics.blit(RenderPipelines.GUI_TEXTURED, RequestMonitorPopup.achievementTextures,
                innerLeftSide - mapX + 97, innerTopSide - mapY - 220, 0.0f, 202.0f, 26, 26, 256, 256,
                ARGB.colorFromFloat(1.0F, 0.7F, 0.7F, 0.7F));
            guiGraphics.text(minecraft.font, s, innerLeftSide - mapX + 111 - minecraft.font.width(s) / 2,
                innerTopSide - mapY - 210, 0xFFFFFFFF, true);
        } else {
            guiGraphics.blit(RenderPipelines.GUI_TEXTURED, RequestMonitorPopup.achievementTextures,
                innerLeftSide - mapX + 97, innerTopSide - mapY - 162, 0.0f, 202.0f, 26, 26, 256, 256,
                ARGB.colorFromFloat(1.0F, 0.7F, 0.7F, 0.7F));
            guiGraphics.text(minecraft.font, s, innerLeftSide - mapX + 111 - minecraft.font.width(s) / 2,
                innerTopSide - mapY - 152, 0xFFFFFFFF, true);
        }
        renderLinkedOrderListItems(guiGraphics, list, innerLeftSide - mapX + 102, innerTopSide - mapY - 180, par1,
            par2);
    ...
    private void renderLinkedOrderListItems(GuiGraphicsExtractor guiGraphics, LinkedLogisticsOrderList list, int xPos,
        int yPos, int par1, int par2) {
        int size = list.size();
        int startLeft = -(size - 1) * (30 / 2) + xPos;
        yPos += 13;
        for (IOrderInfoProvider aList : list) {
            int badgeTint = aList.isInProgress() ?
                ARGB.colorFromFloat(1.0F, 0.1F, 0.9F, 0.1F) :
                ARGB.colorFromFloat(1.0F, 0.7F, 0.7F, 0.7F);
            guiGraphics.blit(RenderPipelines.GUI_TEXTURED, RequestMonitorPopup.achievementTextures, startLeft - 5,
                yPos - 5, 0.0f, 202.0f, 26, 26, 256, 256, badgeTint);
            ...
            if (aList.isInProgress() && aList.getMachineProgress() != 0) {
                ...
                guiGraphics.fill(startLeft - 3, yPos + 21, startLeft - 3 + (22 * aList.getMachineProgress() / 100),
                    yPos + 23, 0xffff0000);
            }
        ...
        if (!list.getSubOrders().isEmpty()) {
            for (int i = 0; i < list.getSubOrders().size(); i++) {
                ...
                renderLinkedOrderListItems(guiGraphics, list.getSubOrders().get(i), startLeft - 20, yPos + 48, par1,
                    par2);
                startLeft += list.getSubOrders().get(i).getTreeRootSize() * (40 / 2);
            }
        }
    ...
    private void renderLinkedOrderListLines(GuiGraphicsExtractor guiGraphics, LinkedLogisticsOrderList list, int xPos,
        int yPos) {
        int size = list.size();
        if (list.isEmpty()) {
            size = 1;
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (!list.getSubOrders().isEmpty()) {
                SimpleGraphics.drawVerticalLine(guiGraphics, startLeft + 8, yPos + 18, yPos + 28, Color.GREEN,
                    zoom.line);
            }
            startLeft += 30;
        }
        ...
        if (!list.getSubOrders().isEmpty()) {
            if (!list.isEmpty()) {
                SimpleGraphics.drawHorizontalLine(guiGraphics, left + 8, startLeft - 22, yPos + 28, Color.GREEN,
                    zoom.line);
                startLeft -= 30;
            }
            SimpleGraphics.drawVerticalLine(guiGraphics, left + ((startLeft - left) / 2) + 8, yPos + 28, yPos + 38,
                Color.GREEN, zoom.line);
            startLeft = xPos + 20 - list.getSubTreeRootSize() * (40 / 2);
            left = startLeft;
            for (int i = 0; i < list.getSubOrders().size(); i++) {
                startLeft += list.getSubOrders().get(i).getTreeRootSize() * (40 / 2);
                SimpleGraphics.drawVerticalLine(guiGraphics, startLeft - 12, yPos + 38, yPos + 48, Color.GREEN,
                    zoom.line);
            ...
            SimpleGraphics.drawHorizontalLine(guiGraphics, left - 12, startLeft - 12, yPos + 38, Color.GREEN,
                zoom.line);
        }
    }

    private void drawPointFor(GuiGraphicsExtractor guiGraphics, LinkedLogisticsOrderList list, int xPos, int yPos,
        int i, int startLeft) {
    ...
    private void renderItemAt(GuiGraphicsExtractor guiGraphics, ItemIdentifierStack item, int x, int y) {
    ...
    private TextureAtlasSprite getTexture(Block blockIn) {
        return Minecraft.getInstance().getModelManager().getBlockStateModelSet()
            .getParticleMaterial(blockIn.defaultBlockState()).sprite();
    }
